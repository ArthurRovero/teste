...
<!--logo-->
    <div class="row top justify-content-center">
        <div class="col-12 text-center">
            ...
            <div class="alert alert-warning col-4 m-auto" style="overflow: hidden;" role="alert">
            Olá: <?php echo $dados['nome'];?></div>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-8 m-auto  border bg-dark">
            <nav class="nav nav-pills nav-justified">
                <a class="nav-item nav-link" href="restrita.php">HOME</a>
...
<div class="conteiner">
    <div class="row">
    <div class="col-2 "></div>
        <div class="col-8 m-auto align-items-center">
        <br>

<!--formulário-->
<div class='container'>
    <div class='row col-8 border rounded bg-dark m-auto '>
<h5 style="color: antiquewhite; text-align:center;">COMENTE O QUE ESTÁ ACHANDO DE NOSSO SITE</h5>
<form action="#" method="POST" id="form">
<input type="text" name="txtnome" class="form-control" value="<?php echo $dados['nome']?>" style="background-color: rgba(37, 35, 35, 0.171); color:aliceblue;"><br>
<textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="txtcoment" placeholder="Comente aqui..." style="background-color: rgba(37, 35, 35, 0.171); color:aliceblue;"></textarea><br>
<div class="col-1 m-auto bg-dark align-items-center"><button type="submit" name="btn" value="Salvar" class="btn btn-primary">Enviar</button></div>
</form>
<br>
<hr id="hr-produtos">
...
//insere valores//
    if(isset( $_POST['btn'])){
        ...
        $sql = "INSERT INTO `tabela_comentarios` (nome, comentario) VALUES ('$nome' , '$coment')";
        $res = mysqli_query( $conn, $sql );
        $qtd = mysqli_affected_rows($conn);
        if($qtd>0){
            echo "<p id='pp'>Sucesso, Foram inseridos $qtd registros";
        }     else{
            echo "<p id='pp'>Erro</p>" . mysqli_error($conn);
        }
    }
...
//deleta comentarios//
    if(isset($_GET['id'])){
        ...
        $id  = $_GET['id'];
        $sql = "DELETE FROM tabela_comentarios WHERE id_tabela_comentario=$id";
        $res = mysqli_query($conn, $sql);
        if($res){
            echo "<p id='pp'>Comentário #$id excluído com sucesso!";
        }
    }

//ordem decrescente//
        $sql = "SELECT * FROM tabela_comentarios ORDER BY id_tabela_comentario DESC";
        $res = mysqli_query($conn, $sql);

//tabela//       
    echo "<table class='table col-8 m-auto table-hover '>
                     <tr id='pp'>
    ...
    while($linha = mysqli_fetch_array($res, 2)){
        ...
        echo "<tr style='font-size: 90%;background-color: white;'>
                <td>$linha[1]</td>
                <td>$linha[2]</td>
                <td>$linha[3]</td>
                <td><a href='?id=$linha[0]'><img src='img/lixo2.png' class='lixo'></a></td>
                <td><a href='edita_comentario.php?id=$linha[0]'><img src='img/lapis.png' class='lapis'></a></td>
             </tr>";
